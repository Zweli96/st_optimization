...
def send_sms_notifications():
    ...
    test_facilities = Facility.objects.filter(
        district=28, name='Test Facility 2').values_list('id', flat=True)
    test_facilities_not = Facility.objects.filter(
        district=28, name='Test District Hospital').values_list('id', flat=True)
    health_workers = Health_Worker.objects.filter(facility__in=test_facilities)
    health_workers_not_visiting = Health_Worker.objects.filter(
        facility__in=test_facilities_not)
...
def send_sms_reminders():
    ...
    facilities = Facility.objects.filter(district=28)
